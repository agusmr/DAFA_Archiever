...
Public Declare Function LzmaCompress Lib "LZMA.dll" (ByRef Dest As Any, _
ByRef destLen As Long, ByRef src As Any, ByVal srcLen As Long, _
...
Public Function LZMACompress_Simple(lpData() As Byte, TheDataOut() As Byte, ByRef nSize As Long, Optional ByVal nLevel As Long = -1) As Boolean
    Dim p As typeLZMApropsEncoded
    Dim ret As Long
    Dim nOldSize As Long
    
    nOldSize = UBound(lpData) + 1

    '/////
    nSize = nOldSize + (nOldSize * 0.01) + 600
    ReDim TheDataOut(nSize + 4)
    ret = LzmaCompress(TheDataOut(5), nSize, lpData(0), nOldSize, p, 5, 5, 0, 3, 0, 2, 32, 1)
    nSize = nSize + 5
